...
class CrossPlatformFileHandler:
    ...
    def safe_move_file(
        ...
        try:
            ...
            self.logger.error(f"Failed to move file from {source_path} to {dest_path}: {e}")
    ...
    def cleanup_all_temp_files(self):
        ...
        self.logger.info(
            f"Cleanup complete: {cleaned} files removed, {failed} failed"
        )
...
def get_file_handler(logger: Optional[logging.Logger] = None) -> CrossPlatformFileHandler:
    """
...
if __name__ == "__main__":
    ...
    temp_file = handler.create_safe_temp_file(
        suffix=".txt",
        content=b"Test content for cross-platform file handling"
    )
